...
if ( ! class_exists( 'BPC_Admin_Page_Class') ) :
 ...
 */
  ...
  class BPC_Admin_Page_Class {
    ...
    public function AddMenuSubPage() {
	  ...
	  $page = add_menu_page( $this->args['page_title'], $this->args['page_title'], $this->args['capability'], $this->createSlug(), array($this, 'DisplayPage'), $this->args['icon_url'], $this->args['position']);
      if (!empty($page)){
         $this->_Slug = $page;
         add_action('load-'.$page, array($this,'Load_page_hooker'));
      }
    ...
    public function DisplayPage() {
      ...
      wp_nonce_field( basename(__FILE__), 'BPC_Admin_Page_Class_nonce' );
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function save($repeater = false) {
    ...
    //check nonce
    if ( ! check_admin_referer( basename( __FILE__ ), 'BPC_Admin_Page_Class_nonce') )
      return;
    ...
    foreach ( $this->_fields as $field ) {
      if(!in_array($field['type'],$skip)){
        ...
        //issue #27
        $validationClass = apply_filters('apc_validattion_class_name', 'BPC_Admin_Page_Class_Validate',$this);
